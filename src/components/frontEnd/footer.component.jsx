...
export default function Footer() {
    ...
    let logo = Content?.data.filter(item => item.id === "logo")[0].value.image
    let address = Content?.data.filter(item => item.id === "footer.addess")[0].value.text
    ...
    return (
        <div className="container-footer >> flex justify-center | w-full border-t-4 border-golden-1 ">
            <div className="footer >> flex flex-col justify-center items-center | p-4 | text-blue-1 | md:flex-row lg:w-7xl lg:items-start">
                <div className="footer__container-logo >> flex justify-center | md:flex-1/3 ">
                    <img className="footer__logo >> w-full object-cover aspect-14/9 | md:w-[250px]" src={logo} alt="logo" />
                </div>
                <div className="footer__contacts >> flex flex-col | p-4 gap-4 | md:gap-8 md:flex-1/3">
                    <h1 className="footer__title-contact >> text-description-3 font-bold ">ที่อยู่</h1>
                    {address}
                </div>
