...
export default function GalleryCar() {
    ...
    return (
        <div id="gallery" className="gallery-car *** flex flex-col justify-center items-center | pt-17 | md:p-0 xl:gap-4  lg:max-w-7xl xl:min-w-7xl lg:snap-center ">

            <div className="gallery-car__container-title *** grid grid-cols-20 w-full items-center | px-4 | text-title-3 font-black text-blue-1  *:items-center *:flex *:justify-center *:h-12 *:md:h-full ">
                <button className={`gallery-car__btn-pre --btn col-span-2  ${parameCarId === 0 && "invisible"}`} onClick={() => { setSearchParame({ "id": parameCarId > 0 ? Car.data[parameCarId - 1].id : Car.data[parameCarId].id }) }}>{`<`}</button>
                <h1 className="gallery-car__title *** select-none text-center col-span-16 text-description-1 h-fit md:text-title-3 xl:text-title-2 text-nowrap truncate">{`${brandName} ${carName}`}</h1>
                <button className={`gallery-car__btn-next --btn col-span-2 ${amount - 1 === parameCarId && " invisible "} `} onClick={() => { setSearchParame({ "id": parameCarId < amount - 1 ? Car.data[parameCarId + 1].id : Car.data[parameCarId].id }) }}>{`>`}</button>
            </div>
            ...
            <div className="gallery-car__container-img *** flex flex-wrap justify-center items-center relative  gap-0 px-4 w-full md:w-[70%] md:gap-4 ">
                ...
                <h1 className="gallery-car__amount-img ***  hidden select-none h-fit absolute right-[26px] top-[8px] z-10 | md:block md:left-[40px] lg:top-8 lg:bottom-[216px] text-white lg:text-2xl font-black text-shadow-black text-shadow-lg/50">{indexBigImg + 1} / {amountImage} </h1>
                ...
                <div className="gallery-car__container-mini-img *** flex flex-row justify-start relative overflow-y-hidden overflow-x-scroll snap-x snap-mandatory scroll-smooth | py-0 gap-2 | md:rounded-none md:pt-0 lg:gap-8">
                    {Imgs.map((img, index) =>
                        <img className={`gallery-car__mini-img --btn *** w-[95%] aspect-16/9 md:aspect-16/9 snap-center rounded-lg object-cover | lg:w-[200px] ${indexBigImg === index ? "" : "md:brightness-60 lg:brightness-40"} hover:brightness-95`} src={img} alt={`${brandName}-${carName}-${index}`} key={`${carName}-${index}`} index={index} onClick={handleOnClickMiniImage} />
                    )}
                </div>
