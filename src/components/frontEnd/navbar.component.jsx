            ...
            <div className="navigation__container >> flex flex-col grow bg-transparent | max-w-7xl  md:h-[60px] | md:items-center md:flex-row">
                <div className="navigation__container-logo >> flex grow justify-between items-center | pr-4 | bg-white md:bg-none | border-b-4 border-golden-1 | md:border-b-0 ">
                    <Link to="/" className=" --btn ">
                        <img className="navigation__logo >> aspect-16/9 object-cover | max-h-[60px]" src={Content?.data[5].value.image} alt="logo" onClick={() => { window.scrollTo(0, 0) }} />
                    </Link>
                ...
                <div className="navigation__menu >> hidden flex flex-col items-end |  p-2 gap-8 grow | bg-white/40 backdrop-blur-lg | md:flex md:flex-row md:justify-end | md:p-0 md:gap-14 md:h-fit | md:bg-transparent " onClick={handleBtnHamburgerMenu}>
                    {title[0]?.value.map(({ text, link }, index) =>
